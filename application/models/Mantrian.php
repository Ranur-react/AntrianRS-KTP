...
class Mantrian extends CI_Model
{
	...
	function viewcount(){
					return $this->db->query("SELECT COUNT(`tb_antrian`.`no`) AS cnt FROM tb_antrian JOIN tb_pasien ON id_ktp=id_pasien  ORDER BY NO ASC;
			")->result_array();
	}
	function viewcountMAX(){
					return $this->db->query("SELECT MAX(`tb_antrian`.`no`) AS Mx FROM tb_antrian ;
			")->result_array();
	}
	function delete($id){
